...
This file is identical for both backends — the API contract (endpoint
paths, request/response JSON shape) is the same whether you're pointed at
the self-hosted PHP + MySQL version or the Vercel + Supabase version. Only
API_BASE_URL and API_SECRET below need to match whichever one you deployed.
...
# Set this to True temporarily when troubleshooting why a device isn't
# matching a license/block you can see in the admin panel — it prints the
# exact HWID string this run computes, every time, so you can diff it
# character-for-character against what's stored server-side. Leave False
# for normal use (customers shouldn't see this).
DEBUG_HWID = False

# --- Configure these for your deployment ---
# PHP version:    "https://yourdomain.com/api"
# Vercel version: "https://your-app.vercel.app/api"
API_BASE_URL = "https://yourdomain.com/api"      # no trailing slash
API_SECRET = "CHANGE_THIS_TO_MATCH_YOUR_SERVER"  # must match API_SECRET on whichever backend you deployed
APP_VERSION = "1.0"
# --------------------------------------------
...
SAVED_KEY_FILES = [
    ...
    "/sdcard/Android/data/com.termux/files/.mytool_key.txt",
    "/sdcard/Android/obb/com.termux/.mytool_key.txt",
]

HWID_FALLBACK_FILES = [
    "/sdcard/.mytool_hwid_fallback",
    os.path.expanduser("~/.mytool_hwid_fallback"),
    "/sdcard/Android/data/com.termux/files/.mytool_hwid_fallback",
    "/sdcard/Android/obb/com.termux/.mytool_hwid_fallback",
]
...
def _slug(value):
    """Keeps only letters/digits — strips spaces/slashes etc. so the value is safe inside a HWID string."""
    return "".join(ch for ch in (value or "") if ch.isalnum())


def get_hwid():
    """
    Main priority: android_id + serial + board + device. If either
    android_id or serial is available, this exact 4-signal combination is
    what gets returned — always, consistently, every run — which is what
    makes a license bound to this HWID on the server recoverable later
    (e.g. after the local cache files are deleted) via the HWID-only lookup
    in check_license(): the server can only find the existing license if
    recomputing get_hwid() reproduces the exact same string that was stored
    when the license was first bound.

    Each individual signal is validated first, so a failed shell command
    (e.g. "cmd: Failure calling service settings: Failed transaction
    (2147483646)") never gets baked into the ID as if it were real data.

    Only if BOTH android_id and serial are unavailable does this fall back
    to a locally persisted random ID (brand/model are pulled in only here,
    purely as a human-readable label — never as part of the main-priority
    combination, since brand/model are model-level, not device-unique, and
    including them there would silently change the HWID for every device
    that already has android_id/serial working).
    """
    ...
    primary = android_id or serial
    if primary:
        parts = [p for p in (android_id, serial, board, device) if p]
        return "_".join(parts)

    # android_id and serial are both unavailable — fall back to a persisted
    # random ID (fetch brand/model only now, purely for a readable label).
    ...
    model = _clean_prop(_run("getprop ro.product.model"))
    label = "_".join(p for p in (_slug(brand), _slug(model)) if p) or "DEVICE"

    new_id = label + "_" + "".join(random.choices(string.ascii_uppercase + string.digits, k=12))
...
def check_license():
    ...
    hwid = get_hwid()
    if DEBUG_HWID:
        print(f"[debug] Device ID (HWID): {hwid}")
    ...
    print("If you expected an existing key to be found automatically, check that this")
    print("Device ID matches the HWID shown for your key in the admin panel.\n")
