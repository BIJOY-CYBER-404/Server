...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    await logActivity('admin_created', { username }, session.username);
  ...
  if (req.method === 'DELETE') {
    ...
    const { data: removed } = await supabaseAdmin.from('admin_users').select('username').eq('id', id).maybeSingle();
    await supabaseAdmin.from('admin_users').delete().eq('id', id);
    await logActivity('admin_removed', { username: removed?.username }, session.username);
    res.status(200).json({ ok: true });
    return;
  }
