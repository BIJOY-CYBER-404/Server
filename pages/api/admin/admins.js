...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // Self-heal: if the is_primary column exists but nothing is flagged
    // primary yet (e.g. the code was upgraded before migration_1.sql ran,
    // or its UPDATE didn't take effect), auto-promote the oldest account
    // so the system always has exactly one primary admin instead of
    // leaving every account stuck looking like a secondary.
    if (admins.length > 0 && !admins.some((a) => a.is_primary)) {
      const oldest = admins[0]; // already ordered oldest-first when none are primary
      await supabaseAdmin.from('admin_users').update({ is_primary: true }).eq('id', oldest.id);
      admins = admins.map((a) => (a.id === oldest.id ? { ...a, is_primary: true } : a));
    }
  ...
  if (req.method === 'POST') {
    // Only the primary admin can add new admin accounts — otherwise any
    // secondary admin could freely mint more admins, which would make the
    // whole primary/secondary distinction meaningless.
    ...
    const { error } = await supabaseAdmin
      .from('admin_users')
      .insert({ username, password_hash: passwordHash, is_primary: false });
  ...
  if (req.method === 'DELETE') {
    ...
    const { data: target } = await supabaseAdmin.from('admin_users').select('is_primary').eq('id', id).maybeSingle();
    if (target?.is_primary) {
      res.status(400).json({ ok: false, error: 'cannot_delete_primary' });
      return;
    }
  ...
  if (req.method === 'PATCH') {
    ...
    // take over the account. Available to primary and secondary admins
    // alike — everyone can always manage their own account.
