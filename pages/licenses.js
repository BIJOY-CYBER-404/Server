...
export default function Licenses({ username }) {
  ...
  useEffect(() => {
    function handleOutsideClick(e) {
      ...
      // The dropdown itself is portaled into document.body (see
      // ActionMenu.js) so it's no longer a DOM descendant of .action-cell
      // — check both explicitly rather than relying only on stopPropagation
      // inside the portal, which doesn't reliably stop this native
      // document-level listener from firing for portal content.
      if (e.target.closest('.action-cell') || e.target.closest('.action-menu')) {
        return;
      }
      setActiveMenu({ id: null, anchor: null });
    }
